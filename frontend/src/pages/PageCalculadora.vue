<template>
  <div class="page-heading">
    ...
    <p>Estimativa por Mifflin-St Jeor para orientar a meta calórica.</p>
  </div>
  ...
  <section class="bmr-panel" aria-label="Calculadora de taxa metabolica basal">
    ...
    <div class="bmr-macros" v-if="perfil.resultado > 0">
      <div class="bmr-macros-header">
        ...
        <p class="bmr-macros-note">Percentuais baseados em diretrizes nutricionais internacionais (AMDR / ISSN).</p>
      </div>
      <div class="bmr-macros-grid">
        <article class="bmr-macro-card bmr-macro-card-proteinas">
          ...
          <p class="bmr-macro-pct">{{ pcts[perfil.objetivo]?.[0] ?? 20 }}% das calorias</p>
        </article>
        <article class="bmr-macro-card bmr-macro-card-gorduras">
          ...
          <p class="bmr-macro-pct">{{ pcts[perfil.objetivo]?.[1] ?? 30 }}% das calorias</p>
        </article>
        <article class="bmr-macro-card bmr-macro-card-carbs">
          ...
          <p class="bmr-macro-pct">{{ pcts[perfil.objetivo]?.[2] ?? 50 }}% das calorias</p>
        </article>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
const form = ref({ sexo: '', peso: 0, altura: 0, idade: 0, objetivo: 'manter' })

watch(perfil, (p) => {
  form.value = { sexo: p.sexo, peso: p.peso, altura: p.altura, idade: p.idade, objetivo: p.objetivo }
}, { immediate: true })
...
function recalcularSeResultado() {
  if (perfil.value.resultado > 0) calcular()
}
